...
async function generatePDFFromPreview() {
    ...
    // First, get geolocation and submit report (while user is still on this tab)
    showStatus('Getting location & submitting report...', 'info');

    try {
        // Get geolocation first (user sees the prompt on this tab)
        let geolocation = null;
        try {
            geolocation = await getGeolocation();
        } catch (error) {
            console.log('Location not available:', error.message);
            // Continue without location - will save to offline queue
        }
        ...
        if (navigator.onLine && geolocation) {
            ...
            try {
                ...
                showStatus('Report submitted! Opening PDF...', 'success');
            } catch (error) {
                saveToOfflineQueue(formData);
                showStatus('Saved offline. Opening PDF...', 'warning');
            }
        } else {
            // Save to offline queue
            saveToOfflineQueue(formData);
            showStatus(geolocation ? 'Saved offline. Opening PDF...' : 'No location - saved offline. Opening PDF...', 'warning');
        }
    } catch (error) {
        console.error('Submit error:', error);
        showStatus('Submit failed. Opening PDF...', 'error');
    }

    // Now generate the PDF (opens new tab)
    generatePDF();
}
