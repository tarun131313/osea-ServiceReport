...
function generatePDFFromPreview() {
    closePreview();
    generatePDF();
    // Note: Report is submitted when user clicks "Print / Save PDF" in the new tab
}
    ...
    printWindow.document.write(`
        ...
        <body>
            <div class="back-bar no-print">
                ...
                <button class="print-btn" onclick="handlePrintClick()">🖨️ Print / Save PDF</button>
            </div>
            <script>
                function handlePrintClick() {
                    // Notify parent window to submit the report
                    if (window.opener && !window.opener.closed) {
                        try {
                            window.opener.submitReportSilent();
                        } catch(e) {
                            console.log('Could not submit to parent:', e);
                        }
                    }
                    // Open print dialog
                    window.print();
