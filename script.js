...
// Generate PDF from preview - also submits report to Google Drive
function generatePDFFromPreview() {
    closePreview();

    // IMPORTANT: Open window FIRST (synchronously) - iOS Safari blocks popups after async operations
    ...
    // Show loading message in new window while we prepare
    printWindow.document.write('<html><head><title>OSEA Service Report</title></head><body style="font-family: sans-serif; padding: 40px; text-align: center;"><h2>Preparing report...</h2><p>Please wait while we submit and generate your PDF.</p></body></html>');

    // Now do async work (geolocation + submit) then write PDF to the already-open window
    prepareAndShowPDF(printWindow);
}

// Async helper function - window is already open so iOS won't block it
async function prepareAndShowPDF(printWindow) {
    showStatus('Getting location & submitting report...', 'info');
    ...
    try {
        ...
        if (navigator.onLine && geolocation) {
            try {
                ...
                showStatus('Saved offline for later sync.', 'warning');
            }
        } else {
            saveToOfflineQueue(formData);
            showStatus(geolocation ? 'Saved offline.' : 'No location - saved offline.', 'warning');
        }
    } catch (error) {
        console.error('Submit error:', error);
        showStatus('Submit failed.', 'error');
    }

    // Now write the PDF content to the already-open window
    writePDFToWindow(printWindow);
}
